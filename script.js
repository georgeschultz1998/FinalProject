  ...
  document.addEventListener('DOMContentLoaded', function() {
    ...
    Papa.parse('database.csv', {
      ...
      complete: function(results) {
        ...
        // Add autocomplete to table
        addAutocompleteToTable();
      }
    });
  });
};
...
var addAutocompleteToTable = function() {
  // Get a reference to the table body element
  var tbody = document.querySelector('#data tbody');

  // Define empty arrays to store the unique values of each column
  var columns = {};
  var rows = tbody.querySelectorAll('tr');

  // Get the header row to use for keys
  ...
  // Loop through each row in the table and add its cell values to the corresponding array
  rows.forEach(function(row, index) {
    if (index > 0) {
      keys.forEach(function(key, i) {
        var cell = row.querySelectorAll('td')[i];
        var value = cell.textContent.trim();
        if (value !== "" && !columns[key].includes(value)) {
  ...
  // Sort the arrays of unique values in ascending order
  ...
  // Add autocomplete to each column
  keys.forEach(function(key) {
    $("#data tbody ." + key.toLowerCase()).autocomplete({
      source: columns[key],
      minLength: 0
    });
  });
  ...
  document.addEventListener('DOMContentLoaded', function() {
    ...
    Papa.parse('database.csv', {
      ...
      complete: function(results) {
        ...
        var buttonDiv = document.querySelector('#searchEnterTable buttonDiv');
